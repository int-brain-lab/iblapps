# -*- coding: utf-8 -*-
"""
Created on Fri Nov 20 09:32:11 2020

@author: Noam Roth
"""
from PyQt5 import QtCore, QtWidgets, QtGui
import numpy as np, brainbox as bb
...
class FilterGroup:
    ...
    def __init__(self):
        ...
        filter_options = [
         'all', 'correct', 'incorrect', 'left', 'right', 'left correct', 'left incorrect', 'right correct', 'right incorrect']
        ...
        self.time_group = QtWidgets.QGroupBox('Bin Spikes (ms):')
        # self.time_text = QtWidgets.QLabel('No. of window  = ')
        self.time_buttons = QtWidgets.QButtonGroup()
        self.time_layout = QtWidgets.QHBoxLayout()
        time_options = ['10','25', '50', '75','100','150','200']
        for i, val in enumerate(time_options):
            ...
            if i == 0:
                ...
                button.setChecked(False)
            self.time_buttons.addButton(button, id=i)
            self.time_layout.addWidget(button)
    ...
    def compute_and_sort_trials(self, stim_contrast):
        ...
        trials_iclf['lines'] = [[0, len(correct_left)],
         [len(correct_left),
          len(correct_left) + len(correct_right)],
         [len(correct_left) + len(correct_right),
          len(correct_left) + len(correct_right) + len(incorrect_left)],
         [
          len(correct_left) + len(correct_right) + len(incorrect_left), len(trials_iclf['trials'])]]
    ...
    def get_sort_method(self, case):
        if case == 'all':
            ...
            if (case == 'correct') | (case == 'incorrect'):
    ...
    def reset_filters(self, stim=True):
        ...
        return (
         stim_contrast, case, sort_method)