# -*- coding: utf-8 -*-
"""
Created on Fri Nov 20 09:30:30 2020

@author: Noam Roth
"""
from PyQt5 import QtGui, QtWidgets
import numpy as np, os, brainbox as bb
...
class DataGroup:
    ...
    def load(self, folder_path):
        ...
        return (
         self.ephys_file_path, self.gui_path)
    ...
    def load_data(self):
       ...
       #self.spikes, self.clusters, self.channels = bbone.load_spike_sorting_with_channel(eid, one=one)
        self.channels = aio.load_object(self.probe_path, 'channels')
        ...
        self.clusterchans = np.array(self.clusters.channels[self.ids])
        self.areas = r.get(self.channels.brainLocationIds_ccf_2017[self.clusterchans])['acronym']
    ...
    def compute_depth_and_amplitudes(self):
        ...
        for clu in self.ids:
            ...
            self.amps = np.append(self.amps, np.mean(units_b.amps[str(clu)]) * 1000000.0)
    ...
    def sort_data(self, order):
        ...
        self.clust_areas = self.areas[order]
    ...
    def compute_peth(self, trial_type, clust, trials_id, align,time_bin):
        if align ==1:
            peths, bin = bb.singlecell.calculate_peths(self.spikes.times, self.spikes.clusters, [
             self.clust_ids[clust]], self.trials[trial_type][trials_id], self.t_before1, self.t_after1, time_bin,
             self.gaussian_smooth)
        elif align==2:
            peths, bin = bb.singlecell.calculate_peths(self.spikes.times, self.spikes.clusters, [
             self.clust_ids[clust]], self.trials[trial_type][trials_id], self.t_before2, self.t_after2, time_bin,
             self.gaussian_smooth)
        peth_mean = peths.means[0, :]
        peth_std = peths.stds[0, :] / np.sqrt(len(trials_id))
        t_peth = peths.tscale
        return (
         t_peth, peth_mean, peth_std)

    def compute_fano(self, trial_type, clust, trials_id, align,time_bin):
        if align==1:
            peths, bin = bb.singlecell.calculate_peths((self.spikes.times), (self.spikes.clusters), [
            self.clust_ids[clust]],
            (self.trials[trial_type][trials_id]), (self.t_before1), (self.t_after1), time_bin,
             self.gaussian_smooth, return_fr=False)
        elif align==2:
            peths, bin = bb.singlecell.calculate_peths((self.spikes.times), (self.spikes.clusters), [
            self.clust_ids[clust]],
            (self.trials[trial_type][trials_id]), (self.t_before2), (self.t_after2),  time_bin,
             self.gaussian_smooth,return_fr=False)
        peth_mean = peths.means[0, :]
        total_num_spikes = np.sum([i*len(trials_id)/10 for i in peth_mean])
        peth_var = np.square(peths.stds[0, :])
        t_peth = peths.tscale
        fano = np.divide(peth_var, peth_mean)
        return (
         t_peth, peth_mean, peth_var, fano, total_num_spikes)

    def compute_rasters(self, trial_type, clust, trials_id, align):
        x = np.empty(0)
        y = np.empty(0)
        ...
        for idx, val in enumerate(self.trials[trial_type][trials_id]):
            if align ==1:
                spks_to_include = np.bitwise_and(spk_times >= val - self.t_before1, spk_times <= val + self.t_after1)
            elif align ==2:
                spks_to_include = np.bitwise_and(spk_times >= val - self.t_before2, spk_times <= val + self.t_after2)
    ...
    def compute_autocorr(self, clust):
        self.clus_idx = np.where(self.spikes.clusters == self.clust_ids[clust])[0]
        x_corr = bb.population.xcorr(self.spikes.times[self.clus_idx], self.spikes.clusters[self.clus_idx], self.autocorr_bin, self.autocorr_window)
        corr = x_corr[0, 0, :]
        return (
         self.t_autocorr, corr)

    def compute_template(self, clust):
        template = self.clusters.waveforms[self.clust_ids[clust], :, 0] * 1000000.0
    ...
    def compute_waveform(self, clust):
        if len(self.ephys_file_path) != 0:
            ...
            return (
             self.t_waveform, wf_mean, wf_std)
    ...
    def compute_timescales(self):
        ...
        self.t_template = 1000.0 * np.arange(n_template) / sr
        n_waveform = np.int(sr / 1000 * self.waveform_window)
        self.t_waveform = 1000.0 * np.arange(n_waveform) / sr