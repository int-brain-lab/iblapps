...
class LoadData:
    ...
    def get_data(self):
        ...
        if self.spike_collection == '':
            ...
            # iblsorter is defaults, then pykilosort, if not present look for normal kilosort
            # Find all collections
            all_collections = self.one.list_collections(self.eid)

            if f'alf/{self.probe_label}/iblsorter' in all_collections:
                self.probe_collection = f'alf/{self.probe_label}/iblsorter'
                probe_path = Path(self.sess_path, 'alf', self.probe_label, 'iblsorter')

            elif f'alf/{self.probe_label}/pykilosort' in all_collections:
